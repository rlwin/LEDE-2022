...
import requests
url='http://ws.audioscrobbler.com/2.0/?method=artist.search&artist=lil&api_key=d9d348af5d849be2e73b78a78f25ca97&format=json&limit=50'
...
# 
# #API key	d9d348af5d849be2e73b78a78f25ca97
...
lil_artists=(data['results']['artistmatches']['artist'])
#print(len(lil_artists))
for artists in lil_artists:
    #print(artists['name'])
    #print(artists['listeners'])
    print(f"{artists['name']} has {artists['listeners']} listners")
...
for listens in list:
    #print(listens['listeners'])
    total_list=(listens['listeners'])
    print(total_list)
    list=total_list
...
for artists in lil_artists:
    print(artists['name'])
    list_url=(artists['image'])
    print(list_url[-2] ['#text'])
    print('-----')
...
for artists in lil_artists:
    print(artists['name'])
    print(artists['mbid'])
    #lil John a95384b1-6aec-468c-ae0d-8c6daf87c4c2
...
import requests
url_liljohn='http://ws.audioscrobbler.com/2.0/?method=artist.getinfo&mbid=a95384b1-6aec-468c-ae0d-8c6daf87c4c2&api_key=d9d348af5d849be2e73b78a78f25ca97&format=json'
response_liljohn=requests.get(url_liljohn)
data_liljohn=response_liljohn.json()
...
#print(data_liljohn['artist'].keys())
print(data_liljohn['artist']['name'])
...
list_tags=(data_liljohn['artist']['tags']['tag'])
...
for artist in lil_artists:
    for mbids in mbid_list:
        print(f"http://ws.audioscrobbler.com/2.0/?method=artist.getinfo&api_key=d9d348af5d849be2e73b78a78f25ca97&format=json&mbid={mbids}")
...
#mbid_url='http://ws.audioscrobbler.com/2.0/?method=artist.getinfo&api_key=d9d348af5d849be2e73b78a78f25ca97&format=json&mbid='

mbid_list=[]
for artists in lil_artists:
    if artists['mbid']:
        mbid_list.append(artists['mbid'])
        mbid_list_three=mbid_list[:3]
#print(mbid_list_three)

for mbid in mbid_list_three:
    mbid_url=(f"http://ws.audioscrobbler.com/2.0/?method=artist.getinfo&api_key=d9d348af5d849be2e73b78a78f25ca97&format=json&mbid={mbid}")
...
for mbid in mbid_list_three:
    ...
    response_mbid=requests.get(mbid_url)
    mbid_data=response_mbid.json()
    mbid_artists=(mbid_data['artist']['name'])
    mbid_tags=(mbid_data['artist']['tags']['tag'])
    print(mbid_artists)
    for names in mbid_tags:
        print(names['name'])
    print('-----')
...
for artists in lil_artists:
    mbid_list_ten=mbid_list[:10]
#print(mbid_list_ten)
rapper = ['hip hop', 'swag', 'crunk', 'rap', 'dirty south', 'hip-hop']

for mbid_ten in mbid_list_ten:
    mbid_url_ten=(f"http://ws.audioscrobbler.com/2.0/?method=artist.getinfo&api_key=d9d348af5d849be2e73b78a78f25ca97&format=json&mbid={mbid_ten}")
    response_mbid_ten=requests.get(mbid_url_ten)
    mbid_data_ten=response_mbid_ten.json()
    
    ten_mbid_artists=(mbid_data_ten['artist']['name'])
    ten_mbid_tags=(mbid_data_ten['artist']['tags']['tag'])
    for genre in ten_mbid_tags:
        ten_tags=(genre['name'])
        
        if ten_tags in rapper:
            print(f"{ten_mbid_artists} is a rapper")
        else:
            print(f"{ten_mbid_artists} is NOT a rapper")
            
    #not sure how to just get one true/false per name :/
